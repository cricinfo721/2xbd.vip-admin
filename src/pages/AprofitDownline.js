...
const AprofitDownline = () => {
    ...
    try {
      ...
      if (status === 200) {
        if (response_users.success) {
          ...
          let table =
            '<td colSpan="6" class="abc"><table width="100%" class="sub-table">';
          response_users.results.forEach((item, index) => {
            table = table + `<tr>`;
            table = table + `<td >${item?.username || null}</td>`;
            table =
              table +
              `<td>${helpers.currencyFormat(
                item?.stakeAmount ? item?.stakeAmount : 0
              )}</td>`;
            table =
              table +
              `<td>${
                Math.sign(item?.amount) !== -1
                  ...
                  : "<span class='text-success'>" +
                    helpers.currencyFormat(Math.abs(item?.amount))
              }</span></td>`;
            table =
              table +
              `<td>${
            ...
            table =
              table +
              `<td>${helpers.currencyFormat(item?.stake?.commission)}</td>`;
            table =
              table +
              `<td>${
                Math.sign(item?.amount) !== -1
  ...
  const getData = async (type = "search") => {
    ...
    let path;
    if (check == "AprofitDownline") {
      path = apiPath.reportDownline;
    } else {
